...
class ActionValidateLocation(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if not location:
            ...
            filepath = DEFAULT_DATA_PATH + "/static/cities_list.json"
...
class ActionSearchRestaurants(Action):
            ...
            if response is not None:
                ...
                if response_json["status"] == "success":
                    ...
                    if location.lower() == city_name.lower():
                        ...
                        response_cuisine = zomato.get_cuisines(city_id)
                        supported_cuisines = [
                            "American",
                            "Chinese",
                            "Italian",
                        ...
                        if len(restaurants_found) > 0:
                            ...
                            if len(restaurant_filtered_budget) > 0:
                                ...
                                response_message = "Showing you top rated restaurants:\n"
                                email_message = "Details of top rated restaurants:\n"
                                ...
                                for index in range(0, number_of_records):
                                    ...
                                    if index < 5:
                                        response_message = (
                                            response_message + "\n   " + str(index + 1) + ". "
                                            + restaurant["name"] + " in " + restaurant["address"] + " has been rated "
                                            + restaurant["rating"] + " out of 5. "
                                            + "Average cost for 2 : " + str(restaurant["avg_cost_for_2"]) + "\n"
                                        )
    ...
    def search_restaurant(self, location="", location_details={}, cuisine_list=[]) -> list:
        ...
        # Fetch restaurants
        # Setting limit to 200 so can filter based on budget afterwards to get some restaurants within budget
    ...
    def filter_restaurant_by_budget(self, budget, restaurant_list) -> list:
        ...
        # Set range based on input budget
        rangeMin = 0
        rangeMax = 99999 # Budget for two, more than this amount should not be expected
...
class ActionSendEmail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        smtp_config = {}
        filepath = DEFAULT_DATA_PATH + "/static/smtp_details.txt"
